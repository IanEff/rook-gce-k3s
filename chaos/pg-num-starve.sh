#!/bin/bash
# rook-gce-k3s — chaos/pg-num-starve.sh
...
# chaos-mesh already injects (applications/infrastructure/chaos-mesh/).
#
...
#
# Usage (run from the Mac, same ceph-gce kubeconfig-context assumption every
# other provisioning/scripts/*.sh makes — no --context flag here on purpose):
